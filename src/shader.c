...
void shader_new(unsigned int *shader, const char *vertex_path, const char *fragment_path, const char *geometry_path)
{
    ...
    if (!shader) {
        PANIC("ERROR: Output parameter \'shader\' can't be NULL\n");
    }
    ...
    if (vert_in == NULL) {
        PANIC("ERROR: Failed to open vertex_path. Provided path was: %s\n", vertex_path);
    }
    ...
    switch (ret) {
        case READ_FILE_OK:
            LOG_MESSAGE("INFO: Read file content from %s\n", vertex_path);
            break;
        case READ_FILE_ERROR:
            PANIC("ERROR: A read error occured while reading from vertex shader file %s\n", vertex_path);
        case READ_FILE_TOOMUCH:
            PANIC("ERROR: Vertex shader file %s is too big\n", vertex_path);
        case READ_FILE_NOMEM:
            PANIC("ERROR: Ran out of memory while reading from vertex shader file %s\n", vertex_path);
    }
    ...
    // Check if file stream was openend correctly
    if (frag_in == NULL) {
        PANIC("ERROR: Failed to open fragment_path. Provided path was: %s\n", fragment_path);
    }
    ...
    switch (ret) {
        case READ_FILE_OK:
            ...
            break;
        case READ_FILE_ERROR:
            PANIC("ERROR: A read error occured while reading from fragment shader file %s\n", fragment_path);
        case READ_FILE_TOOMUCH:
            PANIC("ERROR: Fragment shader file %s is too big\n", fragment_path);
        case READ_FILE_NOMEM:
            PANIC("ERROR: Ran out of memory while reading from fragment shader file %s\n", fragment_path);
    }
    ...
    if (geometry_path != NULL) {
        ...
        // Check if file stream was openend correctly
        if (geometry_in == NULL) {
            PANIC("ERROR: Failed to open geometry_path. Provided path was: %s\n", geometry_path);
        }
        ...
        switch (ret) {
            case READ_FILE_OK:
                ...
                break;
            case READ_FILE_ERROR:
                PANIC("ERROR: A read error occured while reading from geometry shader file %s\n", geometry_path);
            case READ_FILE_TOOMUCH:
                PANIC("ERROR: Geometry shader file %s is too big\n", geometry_path);
            case READ_FILE_NOMEM:
                PANIC("ERROR: Ran out of memory while reading from geometry shader file %s\n", geometry_path);
        }
    }
    ...
    if (!success) {
        PANIC("ERROR: Failed to compile vertex shader. Provided file: %s\n", vertex_path);
    }
    LOG_MESSAGE("INFO: Created vertex shader.\n");
    ...
    if (!success) {
        PANIC("ERROR: Failed to compile fragment shader. Provided file: %s\n", fragment_path);
    }
    LOG_MESSAGE("INFO: Created fragment shader.\n");
    ...
    if (geometry_path != NULL) {
        ...
        if (!success) {
            PANIC("ERROR: Failed to compile geometry shader. Provided file: %s\n", geometry_path);
        }
        LOG_MESSAGE("INFO: Created geometry shader.\n");
    }
    ...
    if (!success) {
        PANIC("ERROR: Failed to link shader program.\nNote: Check shader sources for errors!\n");
    }
    LOG_MESSAGE("INFO: Created shader program.\n");
